...
@WebServlet("/ExecuteUpdateProdottoServlet")
public class ExecuteUpdateProdottoServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!UtilityProdottoForm.validateArticoloBean(prodottoInstance)) {
			request.setAttribute("update_prodotto_attr", prodottoInstance);
			request.setAttribute("errorMessage", "Attenzione sono presenti errori di validazione.");
			request.getRequestDispatcher("/prodotto/insert.jsp").forward(request, response);
			return;
		} 
		try {
			MyServiceFactory.getProdotto_Service_Instance().aggiorna(prodottoInstance);
